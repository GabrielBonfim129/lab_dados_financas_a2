...
- **risk_return_ranking.R**: funções utilitárias para montar o pool de ativos por setor, baixar preços via `yfR`, calcular métricas anualizadas de retorno/volatilidade/Sharpe e rankear os ativos com pesos dependentes do perfil (conservador, moderado ou arrojado). O universo usa os nomes completos de setores da B3 e um conjunto ampliado de tickers para cada um. Também gera séries normalizadas e métricas agregadas de portfólio para a visualização final.
